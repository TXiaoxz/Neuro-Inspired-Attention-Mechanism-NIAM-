...
# Import NIAM (Neuro-Inspired Attention Mechanism)
try:
    from niam import NIAM
    NIAM_AVAILABLE = True
    print("NIAM module found!")
except ImportError:
    print("Warning: niam.py not found!")
    print("Please copy niam.py to the project directory.")
...
class TransformerEnhancer(nn.Module):
    def __init__(self, n_mels=80, d_model=256, nhead=8, num_layers=4, dim_feedforward=1024, dropout=0.1, use_niam=False):
        """
        Transformer-based Audio Enhancer with NIAM
        ...
        Args:
            ...
            use_niam: Whether to use NIAM (Neuro-Inspired Attention Mechanism)
        """
        super().__init__()
        self.n_mels = n_mels
        ...
        if use_niam and NIAM_AVAILABLE:
            ...
            print("\n" + "="*70)
            print("NIAM INTEGRATED INTO TRANSFORMER")
            print("="*70)
        elif use_niam and not NIAM_AVAILABLE:
            print("ERROR: NIAM requested but niam.py not found!")
            print("Please copy niam.py to project directory")
    ...
    def forward(self, x):
        ...
        # Apply NIAM (if enabled)
        if self.use_niam and hasattr(self, 'niam'):
            # NIAM expects (batch, d_model, time) or (batch, time, d_model)
            # Current: (batch, time, d_model) - compatible
            x = self.niam(x)
...
if __name__ == "__main__":
    # MODIFIED: Larger scale for meaningful comparison with zxp
    USE_COCKTAIL_PARTY = COCKTAIL_AVAILABLE  # Auto-detect
    NUM_TRAIN_SAMPLES = 1000  # MODIFIED: Comparable to zxp (his: 5000, ratio 1:5)
    NUM_VAL_SAMPLES = 100     # MODIFIED: Comparable to zxp (his: 500, ratio 1:5)
    BATCH_SIZE = 4            # MODIFIED: CPU limit (zxp uses 16)
    NUM_WORKERS = 0           # MODIFIED: Must be 0 for CPU stability
    NUM_EPOCHS = 10           # MODIFIED: Comparable to zxp (his: 15) to 5
    ...
    print("Creating Transformer model with NIAM...")
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=0.01)  # Same as baseline
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            checkpoint_path = os.path.join(checkpoint_dir, "transformer_niam_best.pt")
            torch.save(model.state_dict(), checkpoint_path)
            print(f"  -> Saved to {checkpoint_path} (best_val={val_loss:.4f})")

    print(f"\n✓ Training complete! Best val loss: {best_val_loss:.4f}")
    print(f"Model saved to: {os.path.join(checkpoint_dir, 'transformer_niam_best.pt')}")
    print("\n" + "="*70)
    print("NIAM Transformer Training Complete")
    print("="*70)
    print("Compare with baseline:")
    print("  Baseline Transformer:  checkpoints/transformer_enhancer_best.pt")
    print("  NIAM Transformer:      checkpoints/transformer_niam_best.pt")
    print("="*70)
    print("\nTarget: +12-15 dB SI-SNR improvement")
    print("Next: Report results to zxp for GPU testing")