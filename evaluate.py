...
Supports CNN, RNN, and Transformer models
"""
import torch
...
def compute_si_snr(estimated, target):
    ...
    Args:
        estimated: Enhanced signal (batch, *)
        target: Clean target signal (batch, *)
...
def compute_snr(estimated, target):
    """
    Compute traditional Signal-to-Noise Ratio (SNR)
...
def evaluate_model(model, dataloader, device):
    ...
    si_snr_noisy_scores = []  # Baseline: noisy input vs clean
    snr_noisy_scores = []

    with torch.no_grad():
        for noisy_mel, clean_mel in tqdm(dataloader, desc="Evaluating"):
            ...
            # Compute metrics for enhanced audio
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate Audio Enhancement Models (CNN/RNN/Transformer)')
    ...
    print(f"  Noisy Input:      {metrics['si_snr_noisy']:.4f} dB")
    print(f"  Enhanced Output:  {metrics['si_snr_enhanced']:.4f} dB")
    print(f"  Improvement:      {metrics['si_snr_improvement']:+.4f} dB")
    print()
    print("SNR Metrics:")
    print(f"  Noisy Input:      {metrics['snr_noisy']:.4f} dB")
    print(f"  Enhanced Output:  {metrics['snr_enhanced']:.4f} dB")
    print(f"  Improvement:      {metrics['snr_improvement']:+.4f} dB")
    print()
