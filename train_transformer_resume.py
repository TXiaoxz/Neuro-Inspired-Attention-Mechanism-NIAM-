...
if __name__ == "__main__":
    ...
    # MODIFIED: Resume from checkpoint if exists
    START_EPOCH = 0
    ...
    if os.path.exists(checkpoint_path):
        ...
        START_EPOCH = 8  # We completed 8 epochs
        print(f"Loaded checkpoint from epoch {START_EPOCH}")
        print(f"Will continue training epochs {START_EPOCH+1} to {NUM_EPOCHS}")
        print("="*70)
    ...
    # If resuming, we need to find the best val loss from previous training
    if START_EPOCH > 0:
        # The best model was saved, so best_val_loss should be recalculated
        # We'll let it update naturally from validation
        print(f"Note: Previous best val loss was -1.5764 (from logs)")
        best_val_loss = -1.5764  # From your training log
    
    for epoch in range(START_EPOCH, NUM_EPOCHS):  # MODIFIED: Start from START_EPOCH
